...
Dygraph.Export.putVerticalLabelY1 = function (ctx, divLabel, options, font, color, textAlign) {
    ...
    if (textAlign == "center") {
        ...
        top = Math.ceil((ctx.canvas.height - textDim.width) / 4 + textDim.width);
    }
...
Dygraph.Export.drawLegend = function (canvas, dygraph, options) {
    ...
    var labelsX = Math.max(Math.floor((canvas.width - labelsWidth) / 2), 0);
    ...
    for (i = 0; i < labels.length; i++) {
        if (labelVisibility[i]) {
            ...
            //     of this file (UTF-8).
            var txt = "- " + labels[i];
            var newX = labelsX + ctx.measureText(txt).width + labelMargin;
